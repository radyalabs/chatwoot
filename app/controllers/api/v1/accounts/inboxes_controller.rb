class Api::V1::Accounts::InboxesController < Api::V1::Accounts::BaseController # rubocop:disable Metrics/ClassLength
  ...
  def index
    @inboxes = policy_scope(Current.account.inboxes.order(created_at: :desc, channel_type: :asc).includes(:channel, { avatar_attachment: [:blob] }))
  end
...
<<<<<<< Updated upstream
     :lock_to_single_conversation, :portal_id, :sender_name_type, :business_name, :channel_status]
=======
     :lock_to_single_conversation, :portal_id, :sender_name_type, :business_name, :greeting_image, :delete_greeting_image]
>>>>>>> Stashed changes
  end
