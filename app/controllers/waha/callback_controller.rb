class Waha::CallbackController < ApplicationController # rubocop:disable Metrics/ClassLength
  ...
  def receive
    ...
    when 'initial_scan'
      ...
      Rails.logger.info "Processing event type: #{event_type} for phone number: #{phone_number}"
    end

    head :ok
  rescue StandardError => e
    Rails.logger.error "Error processing WAHA callback: #{e.message}"
  ...
  def process_initial_scan(phone_number) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/MethodLength
    Rails.logger.info "🚀 Processing initial scan for #{phone_number}"
  ...
  def broadcast_qr_code(channel, _qr_data)
  ...
  def handle_message_phone_mismatch(registered_phone, _sender_phone, payload)
