...
function loadSavedConfiguration() {
  ...
  if (flowData?.agents_config) {
    ...
    if (agent_index !== -1) {
      ...
      // Delay message toggle
      isDelayEnabled.value = config?.delay_enabled === true || config?.delay_enabled === 'true';
...
const AVAILABLE_VARIABLES = computed(() => [
  ...
  { label: t('AGENT_MGMT.REMINDER.APPOINTMENT.VARIABLES.BOOKING.DATE'), value: '{{tanggal_booking}}', mock: '25 Des 2025' },
  { label: t('AGENT_MGMT.REMINDER.APPOINTMENT.VARIABLES.BOOKING.TIME'), value: '{{waktu_booking}}', mock: '14:00 WIB' },
  { label: t('AGENT_MGMT.REMINDER.APPOINTMENT.VARIABLES.SERVICE_NAME'), value: '{{nama_layanan}}', mock: 'Konsultasi Premium' },
  { label: t('AGENT_MGMT.REMINDER.APPOINTMENT.VARIABLES.BOOKING.LOCATION'), value: '{{lokasi}}', mock: 'Klinik Pratama' },
]);
...
async function save() {
  ...
  try {
    ...
    displayFlowData.agents_config[agent_index].configurations.delay_enabled = isDelayEnabled.value;
