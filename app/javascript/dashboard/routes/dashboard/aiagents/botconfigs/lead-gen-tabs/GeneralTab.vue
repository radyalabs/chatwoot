...
watch(
  ...
  newData => {
    ...
    const agentIndex = flowData.enabled_agents.indexOf('lead_gen');

    // If lead_gen agent isn't in the flow, skip
    if (agentIndex === -1) {
...
const agentId = computed(() => {
  return getAgentIdByType('lead_gen');
});
...
function disconnectGoogle() {
  ...
  googleSheetsExportAPI.disconnectAccount()
    .then(() => {
      ...
      props.googleSheetsAuth.spreadsheetUrls.lead_gen.output = '';
...
async function createTicketSheet() {
  try {
    ...
    props.googleSheetsAuth.spreadsheetUrls.lead_gen.output = response.data.spreadsheet_url;
  ...
  <div class="w-full">
    ...
    <div class="flex-1 min-w-0 flex flex-col justify-stretch gap-6">
      <div class="space-y-4">
        ...
        <!-- Sistem Tiket Toggle -->
        <div class="mb-6">
          <label class="block font-medium mb-2">{{ $t('AGENT_MGMT.LEADGENBOT.TICKET.SYSTEM_TITLE') }}</label>
          <p class="text-sm text-gray-500 mb-3">{{ $t('AGENT_MGMT.LEADGENBOT.TICKET.SYSTEM_DESC') }}</p>
        ...
        <div v-if="config.ticketSystemActive" class="mb-6">
          <label class="block font-medium mb-2">{{ $t('AGENT_MGMT.LEADGENBOT.TICKET.CREATE_WHEN') }}</label>
          <p class="text-sm text-gray-500 mb-3">{{ $t('AGENT_MGMT.LEADGENBOT.TICKET.CREATE_WHEN_DESC') }}</p>
          <select 
              ...
              <div class="flex flex-col gap-4">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center">
                    ...
                    <div>
                      <h5 class="text-sm font-medium text-slate-900 dark:text-slate-100">Lead Output Sheet</h5>
