...
RSpec.describe Captain::Copilot::WelcomeMessageService do
  ...
  let(:eligibility_result) { Captain::Copilot::Guards::EligibilityGuard::Result.new(code: nil) }
  let(:fixtures) do
    ...
    eligibility_guard = instance_double(Captain::Copilot::Guards::EligibilityGuard, check: eligibility_result)
    ...
    eligibility_logger = instance_double(Captain::Copilot::Guards::EligibilityGuardLogger, log: true)
  ...
  before do
    ...
    allow(Captain::Copilot::Guards::EligibilityGuardLogger).to receive(:new).and_return(fixtures[:eligibility_logger])
  end
  ...
  describe '#perform' do
    ...
    context 'when the conversation is a group conversation' do
      ...
      it 'skips welcome processing' do
        ...
        expect(Captain::Copilot::Guards::EligibilityGuard).not_to have_received(:new)
    ...
    context 'when eligibility fails with a pre-check failure' do
      let(:eligibility_result) do
        Captain::Copilot::Guards::EligibilityGuard::Result.new(code: :pre_check_failure, failure_reason: 'Limit reached')
      end
